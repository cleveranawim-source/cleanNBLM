...
└── app/                  ← 소스 (Vite + React)
    ...
    └── src/lib/
        ...
        ├── structfill.js     구조 이어 채우기 (배지를 가로지르는 색 경계선을 직선으로 연장해 영역별 채움)
        ├── pillguide.js      배지 안 단서 복원 (배지 = 색조 + 흐린 배경 모델을 맞춰 색조 제거·역블러·포아송 합성)
...
배지가 있으면 배지 전체를 다시 채운다(배지 아래 원본은 흐려져 있어 역산 불가). 둘레가 뚜렷한 두 색(카드·흰 바탕 등)으로
나뉘면 경계선을 직선으로 이어 영역별로 채우고 테두리 음영도 옮긴다(structfill.js). 사진처럼 여러 색이면 배지를
"색조 + 흐린 배경(σ≈5.5)" 모델로 맞춰 색조를 걷어내고 역블러해 배지 밑 물체의 배치를 살린다(pillguide.js).
모델이 맞지 않거나 배지 밑이 매끈하면 조화 보간.
